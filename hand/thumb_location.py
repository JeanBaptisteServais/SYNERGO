...
def thumb_location(thumb, index, major, annular, auricular, crop):
    ...
    printing(thumb, index, major, annular, auricular)                   #Print data finger's

    fingers = [index, major, annular, auricular]                        #Recuperate fingers.
    ...
    end_fingers = [finger[-1][1] for finger in fingers if finger != []] #recuperate fingertips
    [cv2.circle(copy, fingers, 2, (255, 0, 0), 2)
     for fingers in end_fingers]



    thumb_find = len([j for i in thumb for j in i if j == (0, 0)])      #Verify thumb isn't empty
    ...
    if thumb_validation_points == thumb_find:                           #Empty thumb
        ...
        thumb = [j for i in thumb for j in i if j != (0, 0)][-1]        #Recuperate last point thumb

        hand = thumb_localisation(end_fingers, thumb)                   #Thumb localisation compared fingers
